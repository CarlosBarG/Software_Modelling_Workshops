...
class Store:
    ...
    def show_machines_by_price_range(self,  max_price: float):
        """
        Show all arcade machines available in the store within a price range.
        """
        machines_in_range = [machine for machine in self.available_arcades_machines if machine.price <= max_price]
        if machines_in_range:
            print(f"Arcade Machines in Store under {max_price} USD:")
            for machine in machines_in_range:
                machine.show_details()
        else:
            print(f"No arcade machines available in the store under {max_price} USD.")

    
    def show_machines_by_weight_range(self, max_weight: float):
        """
        ...
        """
        machines_in_range = [machine for machine in self.available_arcades_machines if machine.weight <= max_weight]
        
        if machines_in_range:
            print(f"Arcade Machines in Store under {max_weight} kg:")
            for machine in machines_in_range:
                machine.show_details()
        else:
            print(f"No arcade machines available in the store under {max_weight} kg.")
        


    def show_machines_by_power_consumption_range(self, max_power_consumption: float):
        """
        ...
        """
        machines_in_range = [machine for machine in self.available_arcades_machines if machine.power_consumption <= max_power_consumption]
        
        if machines_in_range:
            print(f"Arcade Machines in Store under {max_power_consumption} W:")
            for machine in machines_in_range:
                machine.show_details()
        else:
            print(f"No arcade machines available in the store under {max_power_consumption} W.")
    ...
    def show_machines_by_games_quantity(self,  max_games: int):
        """
        Show all arcade machines available in the store with a specific number of games."""
        machines_by_games = [machine for machine in self.available_arcades_machines if len(machine.games) == max_games]
        
        if machines_by_games:
            print(f"Arcade Machines in Store with {max_games} games:")
            for machine in machines_by_games:
                machine.show_details()
        else:
            print(f"No arcade machines available in the store with {max_games} games.")
